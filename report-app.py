...
def fetch_all(table_name: str, cols: str = "*", page_size: int = 1000) -> pd.DataFrame:
    """Generic paginator for tables that are safe to fetch fully (NOT quota_view)."""
    offset = 0
    all_rows = []
    ...
    while True:
        try:
            ...
            st.error(f"Failed to load '{table_name}' (PostgREST APIError).")
...
def load_traceability():
    return fetch_all("traceability", cols="*")

@st.cache_data(show_spinner=False)
def load_farmers():
    return fetch_all("farmers", cols="*")

@st.cache_data(ttl=300, show_spinner=False)
def load_quota_status_counts():
    """
    Calls your Supabase SQL function:
      public.quota_status_counts()
    which returns: quota_status, count
    """
    try:
        res = supabase.rpc("quota_status_counts", {}).execute()
    except APIError as e:
        payload = e.args[0] if e.args else None
        st.error("Failed to load quota status counts (RPC).")
        st.write("Error payload:", payload)
        st.write("Full exception:", repr(e))
        st.stop()
...
def get_total_traceability_count():
    try:
        ...
        payload = e.args[0] if e.args else None
        st.error("Failed to get total traceability count.")
        st.write("Error payload:", payload)
        st.stop()
...
quota_status_count = load_quota_status_counts()  # ✅ replaces quota_view load

# Optional debug toggle (AFTER load)
if st.sidebar.checkbox("Show raw columns/head (debug)"):
    st.write("trace_df columns:", list(trace_df.columns))
    st.write(trace_df.head())
    st.write("farmers_df columns:", list(farmers_df.columns))
    st.write(farmers_df.head())
    st.write("quota_status_count columns:", list(quota_status_count.columns))
    st.write(quota_status_count.head())
...
farmers_df["area_ha"] = pd.to_numeric(farmers_df["area_ha"], errors="coerce")
...
# Filters
# -------------------------
# Exporter filter (multi-exporter aware, robust)
all_exporters = (
    trace_df["exporter"]
    .dropna()
    .astype(str)
...
def matches_any_exporter(val) -> bool:
...
if trace_df_filtered["area_ha"].notna().any():
    ...
    selected_area_min = st.sidebar.slider(
        "Min Area",
        min_value=0.0,
        max_value=max_area if max_area == max_area else 0.0,  # handle NaN
        value=min_area if min_area == min_area else 0.0
    )
...
# KPI cards
total_net_weight = trace_df_filtered["net_weight_kg"].sum(skipna=True)
total_area = trace_df_filtered["area_ha"].sum(skipna=True)
...
col6.metric(
    "📈 Coverage (%)",
    f"{(total_farmers_in_trace / total_farmers_in_farmers * 100):.1f}%"
    if total_farmers_in_farmers else "N/A"
)
...
# Quota Status Pie (NOW from RPC counts, no quota_view load)
st.subheader("✅ Quota Compliance Status")

# ensure columns exist
quota_status_count = ensure_col(quota_status_count, "quota_status", pd.NA)
quota_status_count = ensure_col(quota_status_count, "count", 0)

# clean types
quota_status_count["count"] = pd.to_numeric(quota_status_count["count"], errors="coerce").fillna(0)

if quota_status_count.empty or quota_status_count["count"].sum() == 0:
    st.info("No quota status counts available.")
else:
    pie_chart = alt.Chart(quota_status_count).mark_arc().encode(
        theta=alt.Theta("count:Q"),
        color=alt.Color("quota_status:N"),
        tooltip=["quota_status", alt.Tooltip("count:Q")]
...
top_farmers = (
    trace_df_filtered
    .groupby("farmer_id")["net_weight_kg"]
    .sum()
...
st.download_button(
    "📥 Download CSV",
    data=trace_df_filtered.to_csv(index=False),
    file_name="traceability_data.csv",
    mime="text/csv"
)
